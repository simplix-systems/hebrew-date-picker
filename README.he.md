...
| `presets` | `boolean \| RangePreset[]` | `false` | מצב טווח: סרגל טווחים מהירים (היום, אתמול, 7/30 הימים האחרונים, החודש/השנה הזו והקודמים + מותאם אישית). "החודש הזה" וכן "החודש/השנה שעברה" הם טווחים מלאים (עברי או לועזי, לפי הלשונית הפעילה — בעברית: א׳ תשרי → כ״ט אלול); "השנה הזו" היא חלון מתגלגל — מאותו יום לפני שנה ועד היום. אפשר להעביר מערך `{ label, range }` להתאמה מלאה. |
